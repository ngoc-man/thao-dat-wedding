...
export default function BackgroundMusic() {
  ...
  useEffect(() => {
    ...
    const closeOutside = event => {
      if (!controls.current?.contains(event.target)) setIsVolumeOpen(false)
    }
  ...
  const canHearMusic = isPlaying && volume > 0

  return <>
    <audio ref={audio} src="/audio/wedding-theme.mp3" loop preload="metadata" />
    <div ref={controls} className="fixed bottom-5 right-5 z-40">
...
function SpeakerIcon() {
  return <svg viewBox="0 0 24 24" className="h-5 w-5 fill-none stroke-current stroke-1.8" aria-hidden="true"><path d="M4 10v4h4l5 4V6l-5 4H4Z" /><path d="M16 9a4 4 0 0 1 0 6M18.5 6.5a7.5 7.5 0 0 1 0 11" /></svg>
}

function MutedSpeakerIcon() {
  return <svg viewBox="0 0 24 24" className="h-5 w-5 fill-none stroke-current stroke-1.8" aria-hidden="true"><path d="M4 10v4h4l5 4V6l-5 4H4Z" /><path d="m16 10 4 4m0-4-4 4" /></svg>
}
