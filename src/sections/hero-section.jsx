...
function CountdownCard({ ceremony, timeLeft }) {
  return (
    <div className="border border-white/30 bg-[#2e1c18]/35 px-2 py-3 backdrop-blur-sm sm:px-3 sm:py-4">
      <p className="font-display text-lg text-[#ffe7d8] sm:text-xl">{ceremony.title}</p>
      <p className="mt-1 text-[10px] tracking-[0.12em] text-white/75 uppercase">{ceremony.date}</p>
      {timeLeft ? <div className="mt-3 grid grid-cols-4 border-t border-white/20 pt-3">
        {Object.entries(timeLeft).map(([unit, value]) => <div key={unit}><strong className="block font-display text-lg sm:text-xl">{String(value).padStart(2, '0')}</strong><span className="text-[7px] tracking-[0.08em] text-white/65 uppercase">{unit}</span></div>)}
      </div> : <p className="mt-3 border-t border-white/20 pt-3 text-xs leading-5 text-[#ffe8da]">Cảm ơn bạn đã dành tình cảm và cùng lưu giữ niềm vui của chúng mình.</p>}
    </div>
  )
}
...
export default function HeroSection() {
  ...
  return (
    <section className="relative flex min-h-svh items-end justify-center bg-[#3f2a26] px-5 pb-12 pt-28 text-center text-white sm:items-center sm:pb-0" id="top">
      <img className="absolute inset-0 h-full w-full object-[50%_70%] object-cover opacity-65 sm:object-[50%_65%]" src="/images/hero-wedding.webp" alt="Cặp đôi trong ngày cưới" fetchPriority="high" decoding="async" />
      <div className="absolute inset-0 bg-gradient-to-b from-[#271612]/45 via-[#271612]/15 to-[#271612]/75" />
